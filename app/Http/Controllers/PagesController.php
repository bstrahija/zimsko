...
class PagesController extends Controller
{
    public function index()
    {
        ...
        $latestGames       = Game::where('status', 'completed')->with(['homeTeam', 'awayTeam'])->orderByDesc('scheduled_at')->limit(6)->get();
        $upcomingGames     = Game::where('status', 'upcoming')->with(['homeTeam', 'awayTeam'])->orderBy('scheduled_at')->limit(6)->get();
