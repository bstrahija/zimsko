<li><x-navigation.link href="{{ \App\Services\Settings::get('general.facebook') }}"
        target="_blank">Novosti</x-navigation.link></li>
...
<li class="group relative">
    ...
    <div class="absolute z-50 hidden w-48 rounded-md bg-white py-2 shadow-xl group-hover:block">
        @foreach (\App\Services\Helpers::currentTeams() as $team)
            <a href="{{ route('teams.show', $team->slug) }}"
                ...
                <span class="mr-2 flex w-6"><img src="{{ $team->logo() }}" alt=""
                        class="max-h-6 max-w-6"></span>
                {{ $team->title }}
            </a>
        @endforeach
    </div>
...
<li><x-navigation.link href="https://www.facebook.com/ZimskoPrvenstvoCK/photos_albums"
        target="_blank">Galerije</x-navigation.link></li>
<li><x-navigation.link href="{{ route('contact') }}">Kontakt</x-navigation.link></li>

@if (auth()->check())
    <li class="border-t border-secondary/40 pt-4 md:hidden"><x-navigation.link
            href="{{ url('admin') }}">Admin</x-navigation.link></li>
