        ...
        <x-ui.card class="mb-8" title="Donacije" subtitle="Ponuda je ograničena">
            @if (isset($products) && count($products) > 0)
                @if (session()->has('message'))
                    <div class="p-4 mb-4 text-green-800 bg-green-100 rounded-md border-l-4 border-green-500 shadow-md">
                        <p class="text-lg font-semibold">{{ session('message') }}</p>
                    </div>
                @else
                    @if (session()->has('error'))
                        <div class="p-4 mb-4 text-red-800 bg-red-100 rounded-md border-l-4 border-red-500 shadow-md">
                    ...
                    <form class="space-y-4" wire:submit.prevent="submit" autocomplete="off">
                        ...
                        <div x-show="showDetails">
                            ...
                            <x-ui.button size="lg" type="submit" variant="primary"
                                class="w-full text-lg font-semibold tracking-wider uppercase transition-colors duration-300 hover:bg-opacity-90">
                                Osiguraj majice
                            </x-ui.button>
                        </div>
