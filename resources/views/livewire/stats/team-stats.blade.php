...
new class extends Component {
    ...
    public function stats()
    {
        ...
        return Stats::teamEventStats(eventId: $selectedEventId);
    }
}; ?>

<x-ui.card class="col mb-8" title="Ekipe" subtitle="Vodeće ekipe tokom turnira">
    <div class="mb-3 flex items-center gap-3">
        ...
        {{-- Loading indicator --}}
        <div wire:loading.delay.flex class="hidden items-center gap-2 text-gray-500">
            ...
            <span class="text-sm font-medium">Učitavanje...</span>
        </div>
    </div>

    @if ($this->stats && count($this->stats))
        <div wire:loading.class="opacity-50 pointer-events-none" x-data="{ activeTab: 'score' }"
            ...
            <div x-show="activeTab === 'score'" class="mt-4">
                <x-stats.teams-tab type="score" :stats="$this->stats" />
            </div>

            <div x-show="activeTab === 'three_points'" class="mt-4">
                <x-stats.teams-tab type="three_points" :stats="$this->stats" />
            </div>

            <div x-show="activeTab === 'field_goals'" class="mt-4">
                <x-stats.teams-tab type="field_goals" :stats="$this->stats" />
            </div>

            <div x-show="activeTab === 'free_throws'" class="mt-4">
                <x-stats.teams-tab type="free_throws" :stats="$this->stats" />
            </div>

            <div x-show="activeTab === 'assists'" class="mt-4">
                <x-stats.teams-tab type="assists" :stats="$this->stats" />
            </div>

            <div x-show="activeTab === 'rebounds'" class="mt-4">
                <x-stats.teams-tab type="rebounds" :stats="$this->stats" />
            </div>

            <div x-show="activeTab === 'steals'" class="mt-4">
                <x-stats.teams-tab type="steals" :stats="$this->stats" />
            </div>

            <div x-show="activeTab === 'blocks'" class="mt-4">
                <x-stats.teams-tab type="blocks" :stats="$this->stats" />
            </div>

            <div x-show="activeTab === 'fouls'" class="mt-4">
                <x-stats.teams-tab type="fouls" :stats="$this->stats" />
            </div>

            <div x-show="activeTab === 'turnovers'" class="mt-4">
                <x-stats.teams-tab type="turnovers" :stats="$this->stats" />
            </div>

            <div x-show="activeTab === 'efficiency'" class="mt-4">
                <x-stats.teams-tab type="efficiency" :stats="$this->stats" />
            </div>
        </div>
    @else
