<div>
    <div class="mb-8 grid grid-cols-1 lg:grid-cols-[1fr_260px]">
        <h1 class="mb-4 text-left font-condensed text-2xl font-bold text-secondary md:text-3xl">
            Rezultati utakmica
        </h1>

        <div class="relative inline-block text-right">
            <select wire:model="selectedEventSlug" wire:change="reloadResults"
                class="block w-full appearance-none rounded-md border border-gray-300 bg-white px-4 py-2 pr-8 leading-tight text-gray-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div x-data="{ open: false }">
                        <div>
                            <a href="{{ route('games.show', $game->slug) }}"
                                class="mb-2 block text-sm text-gray-500 transition-all hover:text-primary">
                                <h2 class="mb-1 text-center font-bold">{{ $game->title }}</h2>
                            ...
                            <div class="flex flex-col items-center gap-3 md:flex-row md:gap-6">
                                <a href="{{ route('teams.show', $game->homeTeam->slug) }}" class="mb-2">
                                    <img src="{{ $game->homeTeam->logo() }}"
                                        class="size-10 rounded-full object-contain shadow-md sm:h-20 sm:w-20 md:size-16"
                                        alt="{{ $game->homeTeam->title }}">
                                </a>
                                <a href="{{ route('games.show', $game->homeTeam->slug) }}"
                                    class="text-center text-base font-bold text-gray-700 transition hover:text-primary sm:text-lg">{{ $game->homeTeam->title }}</a>
                            </div>

                            <div class="pt-3 text-center">
                                <a href="{{ route('games.show', $game->slug) }}"
                                    class="mb-6 block text-lg font-bold sm:text-4xl md:mb-2 md:text-3xl">
                                    <span
                                        class="{{ $game->home_score > $game->away_score ? 'text-primary' : 'text-gray-500' }}">{{ $game->home_score }}</span>
                                    <span class="text-gray-400 md:mx-2">:</span>
                                    <span
                                        class="{{ $game->away_score > $game->home_score ? 'text-primary' : 'text-gray-500' }}">{{ $game->away_score }}</span>
                                </a>

                                <button @click="open = !open"
                                    class="rounded bg-gray-50 px-3 py-1 text-xs font-medium text-gray-500 transition-colors hover:bg-gray-100 focus:outline-none focus:ring-1 focus:ring-gray-200">
                                    <span x-show="!open">Detalji</span>
                                    <span x-show="open">Sakrij</span>
                                </button>
                            </div>

                            <div class="flex flex-col items-center gap-3 md:flex-row-reverse md:gap-6">
                                ...
                                <a href="{{ route('teams.show', $game->awayTeam->slug) }}"
                                    class="text-center text-base font-bold text-gray-700 transition hover:text-primary sm:text-lg">{{ $game->awayTeam->title }}</a>
                            </div>
                        </div>
                        ...
                        <div x-show="open" class="mt-4" x-transition:enter="transition ease-out duration-300"
                            ...
                            <div class="grid grid-cols-2 gap-2 text-center text-sm sm:grid-cols-4 sm:gap-4">
                                <div class="rounded-md bg-gray-100 p-2">
                                    <div class="font-semibold">Q1</div>
                                    <div>
                                        <span
                                            class="{{ $game->home_score_p1 > $game->away_score_p1 ? 'text-green-600 font-semibold' : '' }}">{{ $game->home_score_p1 }}
                                        </span>
                                        -
                                        <span
                                            class="{{ $game->away_score_p1 > $game->home_score_p1 ? 'text-green-600 font-semibold' : '' }}">{{ $game->away_score_p1 }}</span>
                                    </div>
                                </div>
                                <div class="rounded-md bg-gray-100 p-2">
                                    ...
                                    <div>
                                        <span
                                            class="{{ $game->home_score_p2 > $game->away_score_p2 ? 'text-green-600 font-semibold' : '' }}">{{ $game->home_score_p2 }}
                                        </span>
                                        -
                                        <span
                                            class="{{ $game->away_score_p2 > $game->home_score_p2 ? 'text-green-600 font-semibold' : '' }}">{{ $game->away_score_p2 }}</span>
                                    </div>
                                </div>
                                <div class="rounded-md bg-gray-100 p-2">
                                    ...
                                    <div>
                                        <span
                                            class="{{ $game->home_score_p3 > $game->away_score_p3 ? 'text-green-600 font-semibold' : '' }}">{{ $game->home_score_p3 }}
                                        </span>
                                        -
                                        <span
                                            class="{{ $game->away_score_p3 > $game->home_score_p3 ? 'text-green-600 font-semibold' : '' }}">{{ $game->away_score_p3 }}</span>
                                    </div>
                                </div>
                                <div class="rounded-md bg-gray-100 p-2">
                                    ...
                                    <div>
                                        <span
                                            class="{{ $game->home_score_p4 > $game->away_score_p4 ? 'text-green-600 font-semibold' : '' }}">{{ $game->home_score_p4 }}
                                        </span>
                                        -
                                        <span
                                            class="{{ $game->away_score_p4 > $game->home_score_p4 ? 'text-green-600 font-semibold' : '' }}">{{ $game->away_score_p4 }}</span>
                                    </div>
                                </div>
                                ...
                                @if ($game->home_score_p5 || $game->away_score_p5)
                                    <div class="rounded-md bg-gray-100 p-2">
                                        ...
                                        <div>
                                            <span
                                                class="{{ $game->home_score_p5 > $game->away_score_p5 ? 'text-green-600 font-semibold' : '' }}">{{ $game->home_score_p5 }}
                                            </span>
                                            -
                                            <span
