...
<template>
    <div class="grid gap-2">
        <button class="btn btn-secondary" @click="nextPeriod" v-if="game.status === 'in_progress'">Slijedeći period</button>
        <button class="btn btn-error" @click="endGame" v-if="game.status === 'in_progress'">
            Završi utakmicu
        </button>
        <button class="btn btn-secondary" @click="startGame" v-if="game.status !== 'in_progress' && game.status !== 'ended'">
            Započni utakmicu
        </button>
        <button class="btn btn-error" @click="backToPlayers" v-if="game.status !== 'in_progress' && game.status !== 'ended'">
            Natrag
        </button>
        <button class="btn btn-primary" @click="showStats" v-if="game.status !== 'scheduled'">Statistika</button>
    </div>
</template>
