...
function selectPlayer(player) {
    ...
    data.selectedStealPlayer = null;
}

function selectStealPlayer(player) {
    data.selectedStealPlayer = player;
}
...
function isActiveSteal(player) {
    return player.id === data.selectedStealPlayer?.id;
}
</script>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3 class="mb-3 text-2xl font-bold text-center">Odaberi igrača (ukradena lopta)</h3>
                    ...
                    <div class="modal-body">
                        ...
                        <div v-if="data.selectedPlayer" class="mt-8">
                            <h3 class="mb-3 text-2xl font-bold text-center">Ukradeno od</h3>

                            <div class="grid mb-12">
                                <div class="text-center">
                                    <button @click="selectStealPlayer(player)" v-for="player in opponentPlayers"
                                        :key="player.id"
                                        :class="{ 'bg-emerald-600': isActiveSteal(player), 'bg-slate-500': !isActiveSteal(player), hidden: isActive(player) }"
                                        class="px-4 py-2 m-1 text-white rounded-md shadow-md transition-colors duration-200 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-opacity-50">
                                        <p class="text-2xl font-bold">{{ player.number }}</p>
                                        {{ player.name }}
                                    </button>
